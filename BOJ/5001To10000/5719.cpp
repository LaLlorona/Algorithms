...
int main()
{
	...
	while (cin >> num_vertice >> num_edge) {
		...
		// 		cout << "from point " << i << " is " << graph[i][j].second << " " << graph[i][j].first << " ";
		// 	}
		// 	cout << "\n";
		// }
		vector<int> almost_shortest_path = Dijkstra(start_point);
		if (almost_shortest_path[end_point] == INF) {
			cout << "-1\n";
		}
		else {
			cout << almost_shortest_path[end_point] << "\n";
		}
	}
