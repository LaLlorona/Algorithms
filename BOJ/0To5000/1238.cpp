...
int main()
{
	...
	while (cin >> num_vertice >> num_edge) {
		...
		int max_cand = 0;
		vector<int> distance_from_party = dijkstra(party_place);
		for (int i = 0; i < num_vertice; i++) {
			vector<int> distance_from_each_place = dijkstra(i);
			max_cand = max(max_cand, distance_from_party[i] + distance_from_each_place[party_place]);
		}
		cout << max_cand << "\n";
	}
